...
export default function PaymentManagement() {
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `BaoCao_DoanhThu_${new Date().toISOString().slice(0, 10)}.csv`,
    );
      ...
      {isAdmin && adminCharts.revenueByHospital && (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 mb-6">
          <h3 className="text-lg font-bold text-slate-900 mb-6">
            Biểu đồ Doanh thu theo Bệnh viện
          </h3>
            ...
            <div className="flex items-end justify-start h-[240px] gap-6 pt-10 overflow-x-auto pb-2 scrollbar-thin px-4">
              {adminCharts.revenueByHospital.map((data, idx) => {
                const maxRev = Math.max(
                  ...adminCharts.revenueByHospital.map((d) => d.total_revenue),
                  1,
                );
                ...
                return (
                  <div
                    key={idx}
                    className="flex flex-col items-center flex-1 group min-w-[100px] max-w-[120px] relative h-full justify-end"
                  >
                    <div className="opacity-0 group-hover:opacity-100 transition-opacity bg-slate-800 text-white text-xs py-2 px-3 rounded-lg absolute -top-2 left-1/2 -translate-x-1/2 z-50 pointer-events-none whitespace-nowrap text-center shadow-lg">
                      ...
                      <div className="text-amber-300">
                        {Number(data.total_revenue).toLocaleString("vi-VN")} đ
                      </div>
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 mb-6 flex flex-wrap gap-4 items-end">
        {!isAdmin && (
          <div className="flex-1 min-w-[200px]">
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Tìm kiếm (Tên, Mã GD, ID...)"
                className="w-full pl-9 pr-3 py-2.5 border rounded-md text-sm outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-200"
                value={search}
                onChange={(e) => {
        ...
        <div className="flex gap-4">
          <div>
            ...
            <select
              className="border rounded-md px-3 py-2.5 text-sm outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-200"
              value={filterStatus}
              onChange={(e) => {
          ...
          <div>
            ...
            <select
              className="border rounded-md px-3 py-2.5 text-sm outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-200 min-w-[140px]"
              onChange={(e) => {
          ...
          {isAdmin ? (
            <table className="min-w-full text-sm">
              ...
              <tbody>
                ...
                {!loadingPaymentsAdmin &&
                  adminCharts.revenueByHospital.map((p, idx) => (
                    <tr
                      key={idx}
                      className="border-b last:border-0 hover:bg-slate-50/50 transition"
                    >
                      ...
                      <td className="py-3 px-4 text-slate-600">
                        {p.appointment?.service_package ? (
                          <div className="flex flex-col">
                            <span className="font-semibold text-indigo-600 line-clamp-1">
                              {p.appointment.service_package.name}
                            </span>
                          ...
                          <div className="flex flex-col">
                            <span className="font-semibold text-slate-700">
                              {p.appointment?.doctor?.user?.full_name
                                ? `BS. ${p.appointment.doctor.user.full_name}`
                                : "N/A"}
                            </span>
                            <span className="text-xs text-slate-400">
                              Đặt lịch khám
